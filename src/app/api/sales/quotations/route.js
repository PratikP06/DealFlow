...
export async function POST(request) {
  ...
  try {
    ...
    const { customerId, lines, status, deliveryPromiseDate } = data

    if (!customerId || !lines || lines.length === 0) {
      return NextResponse.json({ error: 'Customer and at least one line required' }, { status: 400 })
    }

    const customer = await prisma.customer.findUnique({
      where: { id: customerId },
      include: { discountTierRule: true, priceList: true }
    })
    ...
    const tierMaxDiscount = customer.discountTierRule?.maxDiscountPercent || 0
    ...
    for (const line of lines) {
      ...
      const lineTotal = unitPrice * Number(line.quantity) * (1 - requestedDiscount / 100)
      ...
      processedLines.push({
        ...
        marginPercentSnapshot: marginPercent ? Number(marginPercent) : null,
    ...
    await prisma.auditLog.create({
      data: {
        ...
        actorId: user.id,
        action: status === 'PENDING_APPROVAL' ? 'QUOTE_SUBMITTED' : 'QUOTE_CREATED',
        entityType: 'Quotation',
        entityId: quotation.id,
        details: { source: 'sales_rep', status: status || 'DRAFT' }
      }
    })

    if (status === 'PENDING_APPROVAL' && blendedRiskScore > 0) {
      ...
      for (const rule of approvalRules) {
        const minRisk = Number(rule.minRiskScore)
        const maxRisk = rule.maxRiskScore ? Number(rule.maxRiskScore) : Infinity

        if (blendedRiskScore >= minRisk && blendedRiskScore <= maxRisk) {
          if (rule.requiredRoles.length > 0) {
            for (let i = 0; i < rule.requiredRoles.length; i++) {
              await prisma.approvalStep.create({
                data: {
                  quotationId: quotation.id,
                  approvalRound: 1,
                  stepOrder: i + 1,
                  approverRole: rule.requiredRoles[i],
                  status: 'PENDING'
                }
              })
            }
            ...
            await prisma.quotation.update({
              ...
              data: { approvalRound: 1, status: 'PENDING_APPROVAL' }
            })
          }
          break
        }
